...
export class SqliteStorageBackend implements Storage {
  ...
  async init(): Promise<void> {
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_learned_rules_node ON learned_rules(ast_node_id);
    `);
    ...
    this.db
      .prepare(
        `INSERT INTO learned_rules
           (id, agent_id, description, category, ast_node_id, created_at)
         VALUES (?, ?, ?, ?, ?, ?)
  ...
  async getLearnedRules(
    ...
    if (astNodeId) {
      rows = this.db
        .prepare(
          `SELECT id, description, category, ast_node_id, created_at
           FROM learned_rules
           WHERE agent_id = ? AND (ast_node_id = ? OR ast_node_id IS NULL)
           ORDER BY created_at DESC`
        )
        .all(agentId, astNodeId) as any[];
    } else {
      rows = this.db
        .prepare(
          `SELECT id, description, category, ast_node_id, created_at
