...
import type { IngestionInventory, IngestionInventoryEntry, IngestionProcessor, IngestionRequest, IngestionResponse, SourceArtifact, IngestionFileResult } from "./contracts";

interface IngestionManifest { sourceId: string; checksum: string; version?: string; indexedAt: string; program?: string; }
export interface IngestionServiceOptions { processor: IngestionProcessor; manifestStorage: StorageAdapter; manifestPrefix?: string; inventoryKey?: string; }
...
export class IngestionService {
  ...
  constructor(options: IngestionServiceOptions) { this.processor = options.processor; this.storage = options.manifestStorage; this.prefix = options.manifestPrefix ?? "ingestion/manifests"; this.inventoryKey = options.inventoryKey ?? "ingestion/inventory.json"; }
